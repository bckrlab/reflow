...
class FileCache(Cache):
    ...
    def delete_all(self, step:str, option:Hashable=None) -> None:
        ...
        if option is not None:
            option_hash = self._hash(option_hash)
        ...
        for f in out_path.iterdir():
            if re.match("cache___.*", f.name):
                ...
                if file_step_hash != step_hash:
                    ...
                    and file_option_hash != option_hash:
                    continue
                else:
                    f.unlink()
    ...
    def items(self) -> Iterable[Tuple[List[Tuple[str,Hashable]], Any]]:
        out_path = self._out_path()
        files = list(sorted(
            out_path.glob(f"cache___*___ts-*___seed-*.pickle"), 
            reverse=True))
        last_path = None
        for file in files:
            with open(file, "rb") as f:
