    ...
    def update_options(
        ...
        if options is not None:
            ...
            if type(options) is str:
                if options == "reset":
                    logger.debug(
                        f"{LP}: Reset options: {current_options} -> []")
                    pass
        ...
        if self._not_none_and_existing_option(
            ...
            logger.debug(
                f"{LP}: Setting default option: {default_option}")
            self.steps.nodes[step]["default_option"] = default_option

        if self._not_none_and_existing_option(
                latest_option, current_options, "latest"):
            logger.debug(
                f"{LP}: Setting latest option: {latest_option}")
    ...
    def add_option(
        ...
        if option_parsed in step_dict["options"] and not self.allow_overwrite:
            ...
            if on_exists == "raise":  # only happens if step already existed
                ...
                logger.warn(
                    f"{LP}: Option exists. Overwriting: {option_parsed}")
...
def derive_step_and_option_name(
        func=None,
        step=None,
        option=None,
        default_step=None):
