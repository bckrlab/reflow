...
def filter_recipe(
    ...
    _logger.debug("Filtering recipe")
    ...
    if on_purge_step not in ["raise", "warn", "ignore"]:
        raise ValueError(f"Invalid value for 'on_purge_step': '{on_purge_step}'")
    ...
    for step in recipe.steps.nodes:
        ...
        if len(recipe.steps.nodes[step]["options"]) == 0:
            ...
            new_edges = [(src, dst) for src in predecessors for dst in successors]
...
def all_option_combinations(recipe: Recipe) -> list[Options]:
    ...
    options = [
        ...
        for s in steps
        if s != Recipe._DAG_ROOT
    ]
    combinations = cartesian_product(*options)
    return [dict(zip(steps, c)) for c in combinations]
...
def match_options(
    ...
    return all(
        include_parsed(step, option) for step, option in options.items()
